...
#figure(
  ...
  caption: [Reference raster images generated by the fine-tuned `z-image` pipeline for qualitative raster-vector-raster inspection.],
...
#figure(
  ...
  caption: [Raster-vector-raster fidelity for `vtracer` on raster images generated by the `z-image` pipeline. The evaluation contains 1010 generated PNG images, all of which produced valid SVG files. Metrics are computed by `evaluate_raster_vectorization.py` after rendering the traced SVG back to the input image resolution.],
...
#figure(
  ...
  caption: [Validity, contour alignment, and rendering cost for `vtracer` on `z-image` generated raster inputs. The high 4 px boundary F1 and low Chamfer distance show that most contours are recovered close to their raster positions, while the larger Hausdorff distance indicates occasional distant boundary outliers.],
...
#figure(
  ...
  caption: [SVG complexity for `vtracer` outputs produced from `z-image` generated raster images. The low raster error is obtained with substantially larger SVG files than in the SVG-validation and synthetic-generator evaluations, which suggests that generated-raster artifacts and anti-aliasing are often represented as additional traced path detail.],
) <tab:z-image-raster-vectorization-complexity>

These results indicate that classical tracing is robust on the generated
rasters in the narrow sense of validity and raster reconstruction: all 1010
images produced renderable SVG files, the SSIM is high, and the foreground mask
overlap remains close to the input. At the same time, the resulting SVGs are
large, with more than 1600 path commands on average. For the full pipeline this
is an important trade-off. `vtracer` can preserve the visible raster image
well, but the output may be less editable than a compact vector representation
that models the intended shapes directly.
