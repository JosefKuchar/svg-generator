...
CSV_PATH = Path("results/wandb/floral-glade-79_bgno3qml.csv")
...
def plot_train_loss(df: pd.DataFrame) -> None:
    ...
    fig.tight_layout()
    save_figure(fig, f"{OUTPUT_PREFIX}_train_loss")
    plt.close(fig)
...
def plot_mse_metrics(df: pd.DataFrame) -> None:
    ...
    fig.tight_layout()
    save_figure(fig, f"{OUTPUT_PREFIX}_image_mse")
    plt.close(fig)
